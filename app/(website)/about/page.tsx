...
// 1. We put the main logic inside a separate component
function AboutContent() {
  const [data, setData] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);
  
  // Read the URL to see what section the user clicked
  ...
  // 2. Logic to determine what to show based on the URL
  // If there is no specific section in the URL, show everything
  const showAll = !activeSection; 
  
  // Determine visibility for individual sections
  const renderHero = data.showHero && (showAll || activeSection === "hero");
  const renderMission = data.showMissionVision && (data.mission || data.vision) && (showAll || activeSection === "mission");
  const renderCeo = data.showCeo && data.ceoMessage && (showAll || activeSection === "ceo");
  const renderChairman = data.showChairman && data.chairmanMessage && (showAll || activeSection === "chairman");
  const renderDirector = data.showDirector && data.directorMessage && (showAll || activeSection === "director");

  return (
    <main className="flex flex-col min-h-screen pt-36 pb-20 bg-slate-50">
              ...
              <FadeInView direction="left" className="order-1 lg:order-2">
                <div className="relative w-full max-w-lg mx-auto lg:max-w-none">
                  ...
                  <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden shadow-2xl border border-white/60 group bg-white">
                    {data.heroImage ? (
                      <img src={data.heroImage} alt="About Us" className="w-full h-full object-cover transition-transform duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-105" />
                    ) : (
                      <div className="w-full h-full bg-slate-100 flex items-center justify-center text-slate-400">No Image Provided</div>
                    )}
...
// 3. We export a Suspense wrapper to make Next.js build perfectly
export default function AboutPage() {
  return (
    <Suspense fallback={<div className="min-h-screen pt-36 bg-slate-50 flex items-center justify-center text-slate-400">Loading Content...</div>}>
      <AboutContent />
    </Suspense>
  );
}